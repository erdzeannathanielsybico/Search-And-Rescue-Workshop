...
Uses HSV, not RGB — Hue/Saturation/Value separates a color's identity from lighting brightness, so a threshold tuned once holds up much better than an RGB threshold would as light changes. Supported colors are **YELLOW / GREEN / BLUE** — red was dropped: the classroom itself has a lot of red (clothing, objects, skin tones lean toward the same hue range), so a red target produced constant false-positive detections against the background. Green and blue remain the easiest, most reliable colors to detect indoors (uncommon in backgrounds/skin tones, no hue wraparound). Yellow doesn't wrap around the hue wheel either, but sits close to skin/wood tones — when tuning it, favor raising `S Min`/`V Min` to cut background noise rather than widening `H Min`/`H Max`.
...
2. Press **1 / 2 / 3** to load a starting point for YELLOW / GREEN / BLUE onto the sliders — the last saved range for that color if one exists, otherwise a rough default.
