...
`python3 color_detection_tool_RPi.py`. RPi only — needs a camera attached
locally (cv2.VideoCapture + on-screen windows), so it won't work over SSH
without X11 forwarding, and there's nothing for the laptop to run here.
...
How to start:
  On the RPi, cd into this file's directory (ROS2_Workspace/src/camera_node/camera_node/)
  and run:
    python3 color_detection_tool_RPi.py
